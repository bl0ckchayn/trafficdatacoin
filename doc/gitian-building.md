...
Clone the git repositories for tdc and gitian and then checkout the tdc version that you want to build.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tdc/.git/
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tdc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tdc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tdc-build.assert
```
