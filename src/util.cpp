...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "tdc" is a composite category enabling all TDC-related debug output
            if (ptrCategory->count(string("tdc"))) {
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty tdc.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tdcd.pid"));
