...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TDC address (e.g. %1)").arg("2Qb1kNGfyJLi37bagDQswn2EkGjSCBzokD"));
#endif
...
{
    // return if URI is not valid or is no TDC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert tdc:// to tdc:
    //
    //    Cannot handle this later, because tdc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open tdc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TDC.lnk";
}
...
{
    return GetAutostartDir() / "tdc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a tdc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the tdc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add tdc app to startup item list
