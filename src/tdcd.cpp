...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/tdc.conf are parsed in qt/tdc.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("TDC Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";

        if (mapArgs.count("-version")) {
            strUsage += LicenseInfo();
        } else {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  tdcd [options]                     " + _("Start TDC Core Daemon") + "\n";
    ...
    try {
        ...
        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in tdcd anymore. Use the tdc-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
